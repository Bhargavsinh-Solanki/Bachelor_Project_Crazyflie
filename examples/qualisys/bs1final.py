...
if __name__ == '__main__':
    ...
    # data = ['p1Vals', 'p2Vals' , 'current_xVals' , 'vxVals', 'timeVals']    
                                    
    defined_columns = ['p1','p2','current_x','vx','time']
    df = pandas.DataFrame(columns=defined_columns)
    df['p1'] = p1Vals
    df['p2'] = p2Vals
    df['current_x'] = current_xVals
    df['vx'] = vxVals
    df['time'] = timeVals
    df.to_csv('./cfe8.csv', index=False)

    
    qtm_wrapper.close()

    plt.plot(current_xVals)
    plt.tight_layout()
    plt.show()
